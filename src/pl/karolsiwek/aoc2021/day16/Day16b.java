...
public class Day16b {
    ...
    static class Packet {
        ...
        BigDecimal getDataInt() {
            return BigDecimal.valueOf(Long.parseLong(data,2));
        }

        BigDecimal calculate() {
            switch (typeId) {
                case 0:
                    return subPackets.stream().map(Packet::calculate).reduce((a,b) -> a.add(b)).get();
                case 1:
                    return subPackets.stream().map(Packet::calculate).reduce((a,b) -> a.multiply(b)).get();
                case 2:
                    return subPackets.stream().map(Packet::calculate).reduce((a,b) -> a.min(b)).get();
                case 3:
                    return subPackets.stream().map(Packet::calculate).reduce((a,b) -> a.max(b)).get();
                case 4:
                    return getDataInt();
                case 5:
                    return subPackets.get(0).calculate().compareTo(subPackets.get(1).calculate())>0? BigDecimal.ONE:BigDecimal.ZERO;
                case 6:
                    return subPackets.get(0).calculate().compareTo(subPackets.get(1).calculate())<0? BigDecimal.ONE:BigDecimal.ZERO;
                case 7:
                    return subPackets.get(0).calculate().equals(subPackets.get(1).calculate())? BigDecimal.ONE:BigDecimal.ZERO;
                default:
                    throw new IllegalStateException();
            }
        }
